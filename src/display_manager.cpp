...
M5GFX* DisplayManager::Display = nullptr;
...
void DisplayManager::displayTopBar(const std::string& title, bool submenu, bool searchBar) {
    uint8_t marginX = 4;
    uint8_t marginY = 14;
    ...
    if (searchBar) {
        ...
        // To center text
        offsetX = Utils::getTextCenterOffset(searchQuery, Display->width(), sizeText);
        ...
        std::string formattedTitle = Utils::removeUnknownPrefix(title);
        formattedTitle = Utils::removeCsvSuffix(formattedTitle);

        // To center text
        offsetX = Utils::getTextCenterOffset(formattedTitle, Display->width(), sizeText);
...
void DisplayManager::displaySelection(
    ...
    for (size_t i = 0; i < rowsPerScreen && (currentStartRow + i) < selectionStrings.size(); ++i) {
        ...
        // Main string
        std::string formattedString= Utils::removeUnknownPrefix(selectionStrings[currentIndex]);
        formattedString = Utils::removeCsvSuffix(formattedString);
        Display->setCursor(marginText + margin, startText + stepY * i);

        // Limiter 
        const char* limiter = Utils::isUpperString(formattedString) ? "%.17s" : "%.19s";
...
void DisplayManager::displayScanInfo() {
    Display->fillScreen(BACKGROUND_COLOR);
...
void DisplayManager::displayScanSelection(std::string manufacturerName) {
...
void DisplayManager::displayScanRemote(std::string remoteName) {
    Display->fillRect(1, 70, 184, 19, BACKGROUND_COLOR); //clear
    Display->setTextColor(TEXT_COLOR);
    Display->setTextSize(0.65);
    Display->setCursor(15, 75);
    Display->printf(Utils::removeCsvSuffix("Remote " + remoteName).c_str());
...
void DisplayManager::displayScanCommand(std::string commandName) { 
    Display->fillRect(1, 89, 184, 20, BACKGROUND_COLOR); //clear
    Display->setTextSize(TEXT_MEDIUM);
    Display->setCursor(15, 95);
    Display->setTextColor(PRIMARY_COLOR);
    Display->printf("%.16s", Utils::removeCsvSuffix(commandName).c_str());
}

void DisplayManager::displayScanOver() { 
    Display->fillScreen(BACKGROUND_COLOR);
...
void DisplayManager::displayStringPrompt(std::string stringDescription, std::string stringInput) {
...
void DisplayManager::displayConfirmationPrompt(std::string stringDescription) {
    // Clear
    clearMainView(5);
...
void DisplayManager::drawSearchIcon(int x, int y, int size, uint16_t color) {
    int radius = size / 2;
...
void DisplayManager::drawRemoteIcon(int startX, int startY, int width, int height, uint16_t iconColor, uint16_t buttonColor) {
    // Dessiner la remote
...
void DisplayManager::drawRect(bool selected, uint8_t margin, uint16_t startY, uint16_t sizeX, uint16_t sizeY, uint16_t stepY) {
        // Draw rect
        if (selected) {
...
void DisplayManager::drawSubMenuReturn(uint8_t x, uint8_t y) {
    Display->setTextSize(TEXT_WIDE);
    Display->setTextColor(PRIMARY_COLOR);
